// Check-in programado dos encarregados (mensagens da manhã e da noite).
//
// Quem chama:
//   - o job de respostas pendentes do banco (a cada 1 min, o dia todo) — ver
//     tickMensagensProgramadas em /api/public/hooks/enviar-respostas-pendentes.
//     É o agendador principal: é o único do banco que comprovadamente chega ao
//     servidor (as respostas do bot saem por ele).
//   - o endpoint /api/public/hooks/mensagens-programadas (GitHub Actions como
//     rede de segurança, disparo manual e diagnóstico).
// A reserva por telefone+período+dia (UNIQUE em ai_bot_envios_programados)
 ...
 *
 * Com janela larga (ex.: 07:15–12:00) os horários sorteados iriam até perto do
 * fim dela, e o "bom dia" chegaria às 11h50 para alguns. Limitando o
 * espalhamento, todo mundo recebe no começo da janela (primeiros 90 min), e o
 * restante dela fica como folga.
 */
const ESPALHAMENTO_MAX_MIN = 90;

/**
 * Reserva (linha com sucesso=false) mais velha que isso é resto de uma execução
 * interrompida no meio: o envio nunca foi confirmado. Sem esta limpeza a pessoa
 * ficaria marcada como "já contatada" e não receberia nada naquele período.
 */
const RESERVA_ORFA_MIN = 10;

/** Teto de envios por chamada do job de 1 em 1 minuto (o prazo também limita). */
const BATCH_MAX_TICK = 5;
...
export type OpcoesProgramadas = {
  /** Força um período (teste fora da janela). */
  periodo?: Periodo;
  /** Quantos envios por chamada (padrão 2). */
  batch?: number;
  /** Não envia nem grava, só mostra quem seria contatado. */
  dryRun?: boolean;
  /** Explica, sem enviar nada, quem recebe hoje e por quê. */
  diagnostico?: boolean;
  /**
   * Instante (Date.now()) a partir do qual não se inicia mais nenhum envio.
   * Usado pelo job de 1 em 1 minuto: a chamada precisa terminar rápido, senão o
   * banco encerra a conexão e o Worker é cancelado no meio de um envio.
   */
  prazoEnvioMs?: number;
};

export type ResultadoProgramadas = { status: number; body: Record<string, unknown> };

const resposta = (body: Record<string, unknown>, status = 200): ResultadoProgramadas => ({
  status,
  body,
});

export async function processarMensagensProgramadas(
  opcoes: OpcoesProgramadas = {},
): Promise<ResultadoProgramadas> {
  ...
  if (!config) return resposta({ error: "config_ausente" }, 500);
  if (!config.ativo && !opcoes.diagnostico) return resposta({ idle: true, motivo: "bot_inativo" });
  if (!config.msg_programadas_ativas && !opcoes.diagnostico) {
    return resposta({ idle: true, motivo: "programadas_desativadas" });
  }
  ...
  const forcadoPeriodo = opcoes.periodo === "manha" || opcoes.periodo === "noite";
  ...
  if (!periodoCalc && !opcoes.diagnostico) {
  ...
  if (periodo === "noite" && !noiteAtiva && !forcadoPeriodo && !opcoes.diagnostico) {
    return resposta({ idle: true, motivo: "noite_desativada" });
  }
  ...
  if (!forcadoPeriodo && !opcoes.diagnostico && agoraMin > limiteEnvioMin) {
    return resposta({
      idle: true,
  ...
  if (variacoes.length === 0 && !fallback && !opcoes.diagnostico) {
    return resposta({ idle: true, motivo: "template_vazio" });
  }
  ...
  const { data: autorizados, error: errAut } = await supabaseAdmin
    ...
    .eq("ativo", true);
  if (errAut) return resposta({ error: errAut.message }, 500);
  ...
  // em vez do "bom dia" genérico. É conversa da MANHÃ: a mensagem começa com
  ...
  if (baseAutorizados.length === 0 && !opcoes.diagnostico) {
    return resposta({ idle: true, motivo: isDiaProgramado ? "sem_autorizados" : "dia_nao_programado_sem_follow_up", diaSemana, diasPermitidos });
  }

  // Quem já recebeu hoje neste período (idempotência)
  const { data: jaEnviados } = await supabaseAdmin
    .from("ai_bot_envios_programados")
    .select("id, telefone, sucesso, enviado_em")
    .eq("data_ref", dataRef)
    .eq("periodo", periodo);

  const limiteOrfa = Date.now() - RESERVA_ORFA_MIN * 60_000;
  const orfas = (jaEnviados ?? []).filter(
    (e) => !e.sucesso && new Date(e.enviado_em).getTime() < limiteOrfa,
  );
  if (orfas.length > 0 && !opcoes.diagnostico && !opcoes.dryRun) {
    console.warn(
      `[msg-programadas] liberando ${orfas.length} reserva(s) sem confirmação: ${orfas.map((o) => o.telefone).join(",")}`,
    );
    await supabaseAdmin
      .from("ai_bot_envios_programados")
      .delete()
  ...
  const enviadosSet = new Set(
    (jaEnviados ?? []).filter((e) => !idsOrfas.has(e.id)).map((e) => e.telefone),
  );
  const pendentes = baseAutorizados.filter((a) => !enviadosSet.has(a.telefone));

  if (pendentes.length === 0 && !opcoes.diagnostico) {
    return resposta({ idle: true, motivo: "todos_contatados", periodo, dataRef });
  }
  ...
  // agendador falhar em alguma rodada. Em janela curta (< CATCH_UP_MIN) vale
  // desde o início: todos saem logo na abertura da janela.
    ...
    return resposta({
      ...
      pessoas: (autorizados ?? []).map((a) => {
        ...
        return {
          ...
          sai_na_proxima_rodada:
            naFila && !enviadosSet.has(a.telefone) && (retaFinal || agoraMin >= alvo),
        };
      }),
    });
  }
  ...
  // No job de 1 em 1 minuto o lote acompanha o que falta: dá conta de todos
  // até o fim do espalhamento (ex.: janela de 10 min com 23 pessoas → 3 por
  // minuto). O prazo da chamada ainda pode cortar antes, e o resto sai na
  // rodada seguinte.
  const modoTick = opcoes.prazoEnvioMs !== undefined;
  const batch = modoTick
    ? Math.min(
        BATCH_MAX_TICK,
        Math.max(1, Math.ceil(pendentes.length / Math.max(1, fimEspalhamentoMin - agoraMin))),
      )
    : Math.min(Math.max(Number(opcoes.batch) || 2, 1), 10);
  ...
  for (let idx = 0; idx < lote.length; idx++) {
    ...
    // No job de 1 em 1 minuto o espaçamento maior vem das próprias rodadas.
    if (idx > 0) {
      await sleep(
        modoTick
    ...
    // Passou do prazo desta chamada: não começa outro envio (sai na próxima).
    if (modoTick && Date.now() > opcoes.prazoEnvioMs!) break;
